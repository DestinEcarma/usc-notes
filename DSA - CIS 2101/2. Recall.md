...
Create a function to insert a new element at the end of a linked list. The function should handle the case where the linked list does not yet exist.
...
void insertLast(List* L, int x) {
	...
	if (L != NULL) {
		...
		if (*L == NULL) {
			...
			// Check if allocating a new memory is valid
			if (newNode != NULL) {
				newNode->data = x;
				newNode->next = NULL;
